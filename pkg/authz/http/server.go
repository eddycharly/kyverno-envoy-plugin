...
func NewServer(addr string, dyn dynamic.Interface, p engine.HTTPSource, config Config) server.ServerFunc {
	...
	return func(ctx context.Context) error {
		env, err := kcel.NewEnv(v1alpha1.EvaluationModeHTTP)
		if err != nil {
			return err
		}
...
`
		...
		env, err = env.Extend(cel.Variable("object", httpcel.RequestType))
		if err != nil {
			return err
		}
		inputAst, issues := env.Compile(config.InputExpression)
		if err := issues.Err(); err != nil {
			return err
		}
		inputProgram, err := env.Program(inputAst)
		if err != nil {
			return err
		}
...
`
		...
		env, err = env.Extend(cel.Variable("writer", httpcel.ResponseWriterType))
		outputAst, issues := env.Compile(config.OutputExpression)
		if err := issues.Err(); err != nil {
			return err
		}
		outputProgram, err := env.Program(outputAst)
		if err != nil {
			return err
		}
		// create mux
